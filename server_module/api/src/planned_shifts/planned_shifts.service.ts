...
export class PlannedShiftsService {
  ...
  async listWashers(userId: string) {
  ...
  async update(
    ...
    if (ps.locationId !== admin.locationId) {
      throw new ForbiddenException('Not your location');
    }
  ...
  async assignWasher(
    ...
    if (ps.status === PlannedShiftStatus.CANCELED) {
      throw new ConflictException(
        'Cannot assign washer to canceled planned shift',
      );
    }
    ...
    if (washer.role !== UserRole.WASHER) {
      throw new ForbiddenException('User is not a washer');
    }
    if (washer.locationId !== admin.locationId) {
      throw new ForbiddenException('Washer belongs to another location');
    }
    ...
    if (
      plannedBayId != null &&
      (!Number.isFinite(plannedBayId) || plannedBayId < 1 || plannedBayId > 20)
    ) {
    ...
    return this.prisma.plannedShiftWasher.create({
      ...
      include: {
        washer: { select: { id: true, phone: true, name: true } },
      },
    });
  }
  ...
  async removeWasher(
    ...
    if (!row) {
      throw new NotFoundException('Washer is not assigned to this planned shift');
    }
